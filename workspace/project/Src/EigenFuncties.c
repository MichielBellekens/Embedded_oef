...
void vEigFunCheckButtons( void )
{
	...
	{
		for( uint32_t i = 0; i < sizeof(pOptions) / sizeof(pOptions[0]); i++ )	//loop over all linked lists contained in the options array
		{
...
void vEigFunDrawMenu( void )
{
	...
	{
		uint32_t padding = 3;
		for( uint32_t i = 0; i < sizeof(pOptions) / sizeof(pOptions[0]); i++ )	//iterate over all the linked lists in the options array
		{
...
void vEigFunToggleMenu( void )
{
	for( uint32_t i = 0; i < sizeof(pOptions) / sizeof(pOptions[0]); i++ )	//iterate over all the linked lists
	{
		RadioButtons * temp = pOptions[i];	//temp pointer (otherwise pointer to 1st node gets lost)
...
void vEigFunReadRadioIntoBuffer( const char* filename )
{
	...
	for( uint32_t i = 0; i < sizeof(ucRadiobuff); i++ )	//clear the buffer so we don't see parts of previous image
	{
		ucRadiobuff[i] = 0;
	}
...
void vEigFunReadBmpIntoBuffer( const char* filename )
{
	...
	for( uint32_t i = 0; i < sizeof(ucImgBuffer); i++ )	//clear the ImgBuffer --> don't want parts of prev image
	{
		ucImgBuffer[i] = 0;
	}
